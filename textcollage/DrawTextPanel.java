...
public class DrawTextPanel extends JPanel  {
	...
	public void doMousePress( MouseEvent e ) {
		...
		if (Math.random() > 0.5)         
			s.setBackground(new Color((float)Math.random(),
					(float)Math.random(), 
					(float)Math.random()));  
		...
		theStrings.add(s); 
		// Set this string as the ONLY string to be drawn on the canvas!     
	...
	private void doMenuCommand(String command) {   
		if (command.equals("Save...")) {
			...
			openFile();         
			canvas.repaint(); 
			// (you'll need this to make the new list of strings take effect)     
		}   
			...
			// save a PNG image of the drawing area           
			...
			try {           
				...
				// draws the canvas onto the BufferedImage, not the screen!     
	...
	private void openFile() {    
		...
		try {       
			...
			in.nextLine(); 
			//skip to the next line before read a new line        
